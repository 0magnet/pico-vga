...
func main() {
	...
	// GPIO sampling at strategic lines to track timing
	var gpioLine0 volatile.Register32
	var gpioLine100 volatile.Register32
	var gpioLine200 volatile.Register32
	var gpioLine300 volatile.Register32
	var gpioLine400 volatile.Register32
	var gpioLine479 volatile.Register32
	gpioInPtr := (*volatile.Register32)(unsafe.Pointer(uintptr(0xd0000004)))
	...
	go func() {
		...
		for {
			...
			if irqs&1 != 0 {
				...
				switch line {
				case 0:
					gpioLine0.Set(gpioInPtr.Get() & 0xFFFF)
				case 100:
					gpioLine100.Set(gpioInPtr.Get() & 0xFFFF)
				case 200:
					gpioLine200.Set(gpioInPtr.Get() & 0xFFFF)
				case 300:
					gpioLine300.Set(gpioInPtr.Get() & 0xFFFF)
				case 400:
					gpioLine400.Set(gpioInPtr.Get() & 0xFFFF)
				case 479:
					gpioLine479.Set(gpioInPtr.Get() & 0xFFFF)
				}
	...
	for {
		...
		if f-lastStatusFrame >= 120 {
			...
			// Read GPIO pins 0-15 to see actual pixel output
			gpioIn := (*volatile.Register32)(unsafe.Pointer(uintptr(0xd0000004)))
			pixelPins := gpioIn.Get() & 0xFFFF
			r := pixelPins & 0x1F
			g := (pixelPins >> 6) & 0x1F
			b := (pixelPins >> 11) & 0x1F
			println("GPIO pins:", hex(pixelPins), "R=", r, "G=", g, "B=", b)

			// GPIO samples from strategic lines (captured during render loop)
			g0val := gpioLine0.Get()
			g100val := gpioLine100.Get()
			g200val := gpioLine200.Get()
			g300val := gpioLine300.Get()
			g400val := gpioLine400.Get()
			g479val := gpioLine479.Get()
			println("GPIO line samples: 0=", hex(g0val), "100=", hex(g100val), "200=", hex(g200val))
			println("  300=", hex(g300val), "400=", hex(g400val), "479=", hex(g479val))
			...
			txFull := fstat & 0xF
			txEmpty := (fstat >> 8) & 0xF
			println("FSTAT: txFull=", hex(txFull), "txEmpty=", hex(txEmpty))
			...
			// SHIFTCTRL - critical for comparing with color_run
